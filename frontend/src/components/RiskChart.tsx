...
function RiskChartInner({ equipment }: Props) {
  // Curve is used for the 72h forecast chart + projection stats
  const curveData = useMemo(() => generateRiskCurve(equipment.riskPercent), [equipment.riskPercent]);

  // Sensor chart data (keep it short and cheap)
  const sensorChartData = useMemo(() => {
  ...
  const riskColor = useMemo(() => {
    return (v: number) =>
      v >= 75 ? 'text-accent-red'
        : v >= 50 ? 'text-accent-orange'
          : v >= 25 ? 'text-accent-yellow'
            : 'text-accent-green';
  }, []);
    ...
    <div className="flex flex-col h-full relative" role="region" aria-label="Risk analysis chart">
      ...
      <div className="flex gap-2.5 px-5 py-2" role="group" aria-label="Risk projections">
        {[
          ...
          <motion.div
            key={stat.label}
            // critical: do NOT re-run "mount" animations on every tick
            initial={false}
            animate={{ opacity: 1, y: 0 }}
      ...
      <div className="flex-1 px-4 min-h-0 relative">
        <div
          className="absolute inset-0 mx-4 rounded-xl overflow-hidden pointer-events-none"
          style={{
            background: 'radial-gradient(ellipse at 50% 100%, rgba(199,80,80,0.03) 0%, transparent 60%)',
          }}
        />
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={curveData} margin={{ top: 15, right: 25, left: 5, bottom: 5 }}>
            ...
            <Area
              ...
              fill="riskGradientCinema"
              filter="url(#warmGlow)"
              isAnimationActive={false}   // huge: avoid internal recharts animations per tick
            />
            <ReferenceLine
              x={0}
              stroke="#d4a853"
      ...
      <div className="px-5 py-3">
        ...
        <div className="flex items-center justify-between mb-2.5">
          ...
          <div className="flex items-center gap-1.5">
            <motion.div
              // keep this tiny pulse; it's cheap
              animate={{ opacity: [0.3, 1, 0.3] }}
              transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
              className="w-1.5 h-1.5 rounded-full bg-accent-green status-dot-green"
              aria-hidden="true"
        ...
        <div className="grid grid-cols-3 gap-2 mb-2.5">
          {[
            { icon: <Thermometer size={13} />, label: 'Temperature', value: `${lastSensor?.temp ?? 0}`, unit: 'F', iconClass: 'text-accent-red', valueClass: 'text-accent-red', accent: 'spotlight-red' },
            { icon: <VibIcon size={13} />, label: 'Vibration', value: `${lastSensor?.vib ?? 0}`, unit: 'mm/s', iconClass: 'text-accent-amber', valueClass: 'text-accent-amber', accent: 'spotlight-gold' },
            { icon: <Gauge size={13} />, label: 'Pressure', value: `${lastSensor?.pres ?? 0}`, unit: 'PSI', iconClass: 'text-accent-blue', valueClass: 'text-accent-blue', accent: 'spotlight-blue' },
          ].map((s) => (
            <div
              key={s.label}
              // removed per-tick mount animation
              className={`cinema-panel rounded-lg px-3 py-2 flex items-center gap-2.5 ${s.accent}`}
            >
              <span className={s.iconClass}>{s.icon}</span>
              <div>
        ...
        <GlassPanel className="p-2" accent="blue">
          <div className="h-16" aria-hidden="true">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={sensorChartData} margin={{ top: 4, right: 4, left: 4, bottom: 4 }}>
                <Line type="monotone" dataKey="temp" stroke="#c75050" strokeWidth={1.5} dot={false} isAnimationActive={false} />
                <Line type="monotone" dataKey="vib" stroke="#d4a853" strokeWidth={1.5} dot={false} isAnimationActive={false} />
                <Line type="monotone" dataKey="pres" stroke="#7b9fd4" strokeWidth={1.5} dot={false} isAnimationActive={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </GlassPanel>
...
// memo prevents re-render unless `equipment` reference changes.
// (It still changes when selected machine updates, which is correct.)
